...
import com.webber.androidemoji.adapter.EmotionGridViewAdapter;
import com.webber.androidemoji.adapter.EmotionPagerAdapter;
...
import com.webber.androidemoji.widget.EmotionIndicatorView;
...
public class EmotionComplateFragment extends BaseFragment {
    ...
    private void initEmotion() {
        // 获取屏幕宽度
        int screenWidth = DisplayUtil.getScreenWidthPixels(getActivity());
        // item的间距
        int spacing = DisplayUtil.dp2px(getActivity(), 12);
        // 动态计算item的宽度和高度
        ...
        // 遍历所有的表情的key
        for (String emojiName : EmotionUtil.getEmotionMap(emotion_map_type).keySet()) {
    ...
    private GridView createEmotionGridView(List<String> emotionNames, int gvWidth, int padding, int itemWidth, int gvHeight) {
        ...
        //设置全局点击事件
        gv.setOnItemClickListener(GlobalOnItemClickManagerUtil.getInstance().getOnItemClickListener(getActivity(),emotion_map_type));
        return gv;
    }
